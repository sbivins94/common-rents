...
app.post("/api/v1/renters", async (req: Request, res: Response) => {
  ...
  try {
    const newRenter = await pool.query(
      "INSERT INTO renters (first_name, last_name, email, kinde_id) VALUES ($1, $2, $3, $4) RETURNING *",
      [firstName, lastName, email, kindeId]
    );
...
app.put("/api/v1/renters", async (req: Request, res: Response) => {
